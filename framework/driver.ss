...
#!chezscheme
(library (framework driver aux)
  (export
    iterate
    break/when
    verify-pass-specifications
    verify-pass-specifications2
    verify-iterated-pass-specifications
    verify-iterated-pass-specifications2
  ...
  (define (verify-pass-specifications2 x)
    (syntax-case x (iterate break/when trace)
      [() #t]
      [((iterate . specs) . rest)
       (begin
         (verify-iterated-pass-specifications2 #'specs)
         (verify-pass-specifications2 #'rest))]
      [((break/when pred?) . rest)
       (syntax-violation 'define-compiler
         "break encountered outside of an iteration"
         x #'(break/when pred?))]
      [((pass) . rest) (identifier? #'pass)
       (verify-pass-specifications2 #'rest)]
      [((trace pass) . rest) (identifier? #'trace)
       (verify-pass-specifications2 #'rest)]
      [((pass assemble) . rest) (identifier? #'pass)
       (verify-pass-specifications2 #'rest)]
      [(bad . rest)
       (syntax-violation 'define-compiler
         "invalid pass specification" x #'bad)]
  ...
  (define verify-iterated-pass-specifications2
    (case-lambda
      [(x) (verify-iterated-pass-specifications2 x #f x)]
      [(orig break? x)
       (syntax-case x (iterate break/when trace)
         [()
          (unless break?
          ...
          #t]
         [((break/when pred?) . rest)
          (verify-iterated-pass-specifications2 orig #t #'rest)]
         [((iterate spec1 spec2 ...) . rest)
          (verify-iterated-pass-specifications2 #'(spec1 spec2 ...))
          (verify-iterated-pass-specifications2 orig break? #'rest)]
         [((pass) . rest) (identifier? #'pass)
          (verify-iterated-pass-specifications2 orig break? #'rest)]
         [((trace pass) . rest) (identifier? #'pass)
          (verify-iterated-pass-specifications2 orig break? #'rest)]
         [((pass assemble) . rest) (identifier? #'pass)
          (syntax-violation 'define-compiler
            "emit pass encountered during iteration"
...
(library (framework driver)
  ...
  (define-syntax run-emit-pass2
    (syntax-rules ()
      [(_ input assemble pass)
       (let ([inv input])
         (let ([output (assemble (lambda () (pass inv)))])
           (void)))]))

  (define-syntax run-iterated-pass2
    (syntax-rules ()
      [(_ input . specs)
       (call-with-current-continuation
         (lambda (k)
           (let loop ([x input])
             (loop (compose-passes2 k x . specs)))))]))

  (define-syntax compose-passes2
    (syntax-rules (iterate break/when trace)
      [(_ k input) input]
      [(_ k input (iterate . specs) . rest)
       (compose-passes2 k
         (run-iterated-pass2 input . specs) . rest)]
      [(_ k input (break/when pred?) . rest)
       (begin
         (when (not (syntax->datum #'k))
           (syntax-violation 'define-compiler
             "break encountered outside of iterate clause"
         ...
         #t)
       (compose-passes2 k
         (let ([inv input]) (if (pred? inv) (k inv) inv)) . rest)]
      [(_ k input (pass) . rest)
       (compose-passes2 k
         (run-pass2 input pass) . rest)]
      [(_ k input (trace pass) . rest)
       (let ([pass (trace-lambda pass (i) (pass i))])
         (compose-passes2 k input (pass) . rest))]
      [(_ k input (pass assemble) . rest)
       (begin
         (when (syntax->datum #'k)
           (syntax-violation 'define-compiler
             "unexpected emit pass inside iteration"
         ...
         (when (not (null? (syntax->datum #'rest)))
           (syntax-violation 'define-compiler
             ...
             #'(pass assemble)))
         #t)
       (run-emit-pass2 input assemble pass)]))

  (define-syntax define-compiler-aux2
    (syntax-rules ()
      ((_ (name) . specs)
       (verify-pass-specifications2 #'specs)
       (define (name input)
         (compose-passes2 #f input . specs)))))

  (define-syntax rewrite-specs2
    (syntax-rules (iterate trace break/when %)
      [(_ name (specs ...))
       (define-compiler-aux2 (name) specs ...)]
      [(_ name (specs ...) (iterate spec1 spec2 ...) rest ...)
       (rewrite-specs2 name
         (specs ... (iterate spec1 spec2 ...) rest ...))]
      [(_ name (specs ...) (trace pass foo ...) rest ...)
       (rewrite-specs2 name
         (specs ... (trace pass foo ...) rest ...))]
      [(_ name (specs ...) (break/when foo ...) rest ...)
       (rewrite-specs2 name
         (specs ... (break/when foo ...) rest ...))]
      [(_ name (specs ...) (pass foo ...) rest ...)
       (rewrite-specs2 name (specs ... (pass foo ...)) rest ...)]))

  (define-syntax define-compiler
    (syntax-rules ()
      [(_ (name) spec1 spec2 ...)
       (rewrite-specs2 name () spec1 spec2 ...)]
      [(_ (name (wrapper-proc)) spec1 spec2 ...)
       (rewrite-specs name wrapper-proc () () spec1 spec2 ...)]
      [(_ (name name-step) spec1 spec2 ...)
       (begin
         (define-compiler (name) spec1 spec2 ...)
