...
;; modified on Feb 28, 2015 by Keyan Zhang
;; - prints all the unexpected stuffs at the end, with the
;;   corresponding source codes and error messages (if applicable)
;; - call `(show-issues)` in the REPL will do the same thing

;; modified on Dec 30, 2016 by Rnic
;; - test-driver
;;   compare the execution results of valid tests
;;   with Chezcsheme results
...
(define (init-fail-list)
  (set! fail-list '()))

(define (update-fail-list num)
  (set! fail-list `(,@fail-list ,num)))

(define (show-issues)
  (if (null? fail-list)
      (printf "cool sir, you look handsome today\n")
      (for-each
         (lambda (x)
           (let ([valid-num (cdr x)]
                 [invalid-num (- (cdr x) (length (valid-tests)))])
             (if (car x)
                 (begin
                   (printf "----------------------------\nFailure #~s: \n" (cdr x))
                   (pretty-print (test-ref (valid-tests) valid-num))
                   (newline)
                   (display-test-failure (cdr x)))
                 (begin
                   (printf "----------------------------\nunexpected pass #~s: \n" (cdr x))
                   (pretty-print (test-ref (invalid-tests) invalid-num))
                   (newline)))))
         fail-list)))
...
(define (find-test num)
  (if (and (null? (valid-tests))
           (null? (invalid-tests))) (load (load-test-suite)))
  ;; (load (load-test-suite))
  (let* ([vt (valid-tests)]
         [it (invalid-tests)]
         [test-suite (append vt it)])
    (test-case-source (list-ref test-suite num))))
  ...
  (begin
    ...
    (let ((compiler (test-compiler))
      ...
      (begin
        ...
        (print-group-heading)
        (for-each (test-one compiler runner #t) (test-suite))

        ;; Process the invalid tests
        (test-suite it)
        (printf "\nTesting (invalid-tests)\n")
        (print-group-heading)
        (for-each (test-one compiler runner #f) (test-suite))
  ...
  (begin
    ...
    (let ((compiler (test-compiler))
      ...
      (begin
        ...
        (printf "Testing (valid-tests)\n")
        (print-group-heading)
        (for-each (test-one-compile compiler runner #t) (test-suite))
...
(define (run-tests)
  (begin
    ...
    (let ((compiler (test-compiler))
      ...
      (begin
        (print-group-heading)
        (for-each (test-one compiler runner #t) suite)
        (print-finalization runner)))))
...
(define (test-one compiler runner valid?)
  (lambda (input)
    (let ((pr (guard (x [else x])
      ...
      (begin
        (print-individual-completion pr runner valid?)
        (record-test-result pr expected-failure runner)))))

(define (test-one-compile compiler runner valid?)
  (lambda (input)
    (let ((pr (guard (x [else x])
                (compiler (test-case-source input))))
          (expected-failure
            (not (test-case-valid? input))))
      (let* ((vt (test-case-source input))
             (chezscheme-eval (for-each eval vt))
             )
        (system (format "./~a > ~a.compile" (output-file) (output-file)))
        (write-file (format "~a.ans" (output-file)) chezscheme-eval)
        (let* ([pr (system (format "diff ~a.compile ~a.ans > ~a.diff" (output-file) (output-file) (output-file)))])
          (system (format "rm ~a.compile ~a.ans > /dev/null" (output-file) (output-file)))
          (begin
            (print-individual-completion pr runner valid?)
            (record-test-result pr expected-failure runner)))))))
...
(define (print-individual-completion pr runner valid?)
  (let ([result (result->string pr)])
    (if valid?
        (begin
          (unless (equal? result (list "Pass" ""))
            (update-fail-list
              `(,valid? . ,(current-test-number runner))))
          (apply printf "~4d    ~8a~a~n"
            (current-test-number runner)
            result))
        (begin
          (when (equal? result (list "Pass" ""))
            (update-fail-list
              `(,valid? . ,(current-test-number runner))))
          (apply printf "~4d    ~8a~a~n"
            (current-test-number runner)
...
(define (write-file file-name vt)
...
(define (result->string pass-result)
  (cond
    ...
    [(eq? pass-result 1)
     (list "Fail"
       (format "~a" "result not correct error"))]
    [else (list "Pass" "")]))
...
(define (record-test-result pass-result expected-fail runner)
  (cond
    ((or (error? pass-result)
         ...
         (and (eq? pass-result 1) (set! pass-result #f)))
     (begin
