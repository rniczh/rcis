...
(valid
       ...
       (let ((a 2))
         (if (if ((lambda (x)
                 ...
                 (if (eq? a (void))
                     ...
                     #f)
                 #f)
             778477 ; #36rgood        ; dyb: cannot use symbols, so use radix 36
             14629  ; #36rbad
           ))))
  ...
  ((let ([v (make-vector 2)])
     ...
     (vector-set! v 1 11)
     (let ([x 2])
       (div (vector-ref v 1) (div 6 (vector-ref v 0))))))

  ((let ([v (make-vector 2)])
     (vector-set! v 0 2)
     (vector-set! v 1 11)
     (let ([x 2])
       (mod (vector-ref v 1) (div 6 (vector-ref v 0))))))
